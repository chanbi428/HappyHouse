...
public class ShopServiceImpl implements ShopService {
	...
	@Override
	public List<ShopDto> dijkstra(Map<String, String> map, ArrayList<Integer> arr) {
		List<ShopDto> tempList = null;
		try {
			tempList = dao.getShops(map);
		} catch (SQLException e) {
			System.out.println("다익스트라 오류 발생");
			e.printStackTrace();
		}
		...
		try {
			List<ShopDto> afterList = Dijkstra.dijkstra(beforeList);
		} catch (Exception e) {
